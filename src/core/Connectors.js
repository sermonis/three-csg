import { Vector, Line3D, Matrix4x4, OrthoNormalBasis, Plane } from 'math';
...
ConnectorList._fromPath2DTangents = function (path2D, start, end) {
    ...
    let result = new ConnectorList([new Connector(path2D.points[0], start, ConnectorList.defaultNormal)]);
    // middle points
    path2D.points.slice(1, pathLen - 1).forEach(function (p2, i) {
        axis = path2D.points[i + 2].minus(path2D.points[i]).toVector3D(0);
        result.appendConnector(new Connector(p2.toVector3D(0), axis, ConnectorList.defaultNormal));
    }, this);
    result.appendConnector(new Connector(path2D.points[pathLen - 1], end, ConnectorList.defaultNormal));
...
ConnectorList._fromPath2DExplicit = function (path2D, angleIsh) {
    ...
    let result = new ConnectorList(
        path2D.points.map(function (p2, i) {
            return new Connector(p2.toVector3D(0), Vector.Create(1, 0, 0).rotateZ(getAngle(angleIsh, p2, i)), ConnectorList.defaultNormal);
        }, this)
    );
    result.closed = path2D.closed;
    return result;
    ...
    followWith: function (cagish) {
        ...
        this.connectors_.forEach(function (connector, notFirst) {
            ...
            if (notFirst || this.closed) {
                ...
                polygons.push.apply(polygons, currCag._toPlanePolygons({ toConnector: connector, flipped: true }));
            }
            if (notFirst === this.connectors_.length - 1 && !this.closed) {
                // build end wall
                polygons.push.apply(polygons, currCag._toPlanePolygons({ toConnector: connector }));
            }
            prevCag = currCag;
...
export { Connector, ConnectorList };
