import { CSG, Connector, fromPolygons, fromPoints } from 'core';
import { OrthoNormalBasis, Vector, Matrix4x4, Path2D } from 'math';
import {
...
function rotate_extrude(params, baseShape) {
    ...
    for (let i = 1; i < segments + 1; i++) {
        ...
        if (Math.abs(angle) < 360) {
            ...
            const endMatrix = Matrix4x4.rotationX(90).multiply(Matrix4x4.rotationZ(-startAngle));
            const endCap = sideShape._toPlanePolygons({ flipped: flipped }).map((x) => x.transform(endMatrix));

            const startMatrix = Matrix4x4.rotationX(90).multiply(Matrix4x4.rotationZ(-angle - startAngle));
...
export { extrudeInOrthonormalBasis, extrudeInPlane, extrude, linear_extrude, rotate_extrude, rotateExtrude, rectangular_extrude };
